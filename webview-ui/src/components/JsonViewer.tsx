...
interface Token { tokenType: TokenKind; value: string; }
...
function tokenize(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if (/[ \t\n\r]/.test(ch)) {
      ...
      tokens.push({ tokenType: 'ws', value: ws });
      continue;
    }
    ...
    if (ch === '"') {
      ...
      tokens.push({ tokenType: input[j] === ':' ? 'key' : 'string', value: str });
      continue;
    }
    ...
    if (ch === '-' || (ch >= '0' && ch <= '9')) {
      ...
      tokens.push({ tokenType: 'number', value: num });
      continue;
    }

    // keywords
    if (input.startsWith('true', i))  { tokens.push({ tokenType: 'boolean', value: 'true' });  i += 4; continue; }
    if (input.startsWith('false', i)) { tokens.push({ tokenType: 'boolean', value: 'false' }); i += 5; continue; }
    if (input.startsWith('null', i))  { tokens.push({ tokenType: 'null',    value: 'null' });  i += 4; continue; }

    tokens.push({ tokenType: 'punc', value: ch }); i++;
  }
  return tokens;
}
        ...
        <pre className={`json-box${isError ? ' is-error' : ''}`}>
          {tokens
            ? tokens.map((t, i) =>
                t.tokenType === 'ws'
                  ? t.value
                  : <span key={i} className={`jt-${t.tokenType}`}>{t.value}</span>
              )
            : raw}
        </pre>
      ) : (
